...
void order()
{
    ...
    while ((continueOrder == 'Y' || continueOrder == 'y') && cart_count < 100) {
        ...
        while (!found) {
            ...
            for (int i = 0; i < 10; i++) {
                if (item_code[i] == code) {
                    ...
                    cout << setfill(' ') << setw(91) << "" << GREEN << BOLD << "Item: " << RESET << item_name[i] << GREEN << " | Price: " << fixed << setprecision(2) << RESET << "RM "<< item_price[i] << RESET << endl;
                    
                    // Ask if user wants to add to cart
                    cout << setfill(' ') << setw(91) << "" << YELLOW << BOLD << "Do you want to add this item to cart? (Y/N): " << RESET;
                    cin >> addToCart;
                    ...
                    if (addToCart == 'Y' || addToCart == 'y') {
                        ...
                        cout << setfill(' ') << setw(91) << "" << BLUE << BOLD << quantity << " " << item_name[i] << " added to cart." << RESET << endl;
                    }
            ...
            if (!found) {
                cout << setfill(' ') << setw(91) << "" << RED << BOLD << "Item code not found. Please check the catalog and try again." << RESET << endl;
                cout << endl;
            }
        }
        ...
        if (cart_count < 100) {
            ...
            do {
                cout << setfill(' ') << setw(91) << "" << YELLOW << BOLD << "Would you like to add a different items? (Y/N): " << RESET;
                cin >> continueOrder;
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
